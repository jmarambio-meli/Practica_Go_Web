...
func (repo *sliceRepository) GetTotalByConsumer(id []int) ([]domain.Producto, float64, error) {
	var totalProductos []domain.Producto

	var sum float64
	for _, v := range id {
		producto, err := repo.storage.GetProductById(v)
		if err != nil {
			return totalProductos, 0, errors.New("error en la busqueda de un ID inexistente")
		}
		if !producto.Is_published {
			return totalProductos, 0, fmt.Errorf("el producto con el id: %v no esta publicado", producto.Id)
		}
		if producto.Quantity <= 0 {
			return totalProductos, 0, fmt.Errorf("no queda stock para la cantidad solicitada del producto con el id: %v ", producto.Id)
		}
		producto.Quantity--
		repo.storage.EditProduct(producto)
		totalProductos = append(totalProductos, producto)
		sum += producto.Price
	}

	if len(totalProductos) <= 10 {
		sum = sum * 1.21
	}
	if len(totalProductos) > 10 && len(totalProductos) <= 20 {
		sum = sum * 1.17
	}
	if len(totalProductos) > 20 {
		sum = sum * 1.15
	}

	return totalProductos, sum, nil
}
