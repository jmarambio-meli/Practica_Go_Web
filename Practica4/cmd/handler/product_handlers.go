...
func (handler *ProductsHandler) EditProduct() gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		product, err := handler.service.EditList(producto, id)
		if err != nil {
			c.String(http.StatusInternalServerError, err.Error())
			return
		}
...
func (handler *ProductsHandler) PatchProduct() gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		product, err := handler.service.PatchList(producto, id)
		if err != nil {
			c.String(http.StatusInternalServerError, err.Error())
			return
		}
...
func (handler *ProductsHandler) DeleteProduct() gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		err = handler.service.DeleteList(id)
		if err != nil {
			c.String(http.StatusInternalServerError, "Error al buscar el producto")
			return
		}
		c.JSON(http.StatusOK, "Producto Borrado Exitosamente")
	}
}
